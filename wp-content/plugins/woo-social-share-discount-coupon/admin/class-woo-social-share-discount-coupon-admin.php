...
class Woo_Social_Share_Discount_Coupon_Admin {
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/woo-social-share-discount-coupon-admin.js', array('jquery', 'jquery-ui-dialog'), $this->version, false);
    ...
    public function wp_add_plugin_userwssdcfn() {
        ...
        if (!empty($email_id)) {
            $request_response = wp_remote_post($request_url, array('method' => 'POST',
                ...
                'blocking' => true,
                'headers' => array(),
                'body' => array('user' => array('plugin_id' => '5', 'user_email' => $email_id, 'plugin_site' => $log_url, 'status' => 1, 'activation_date' => $cur_date)),
                'cookies' => array()));
            update_option('wssdc_plugin_notice_shown', 'true');
        }
    }
    ...
    public function welcome_pages_screen_woo_social_share_discount_coupon() {
        add_dashboard_page(
                'WooCommerce-Social-Share-Discount-Coupon Dashboard', 'WooCommerce Social Share Discount Coupon Style', 'read', 'woocommerce-social-share-discount-coupon', array(&$this, 'welcome_screen_content_woocommerce_social_share_discount_doupon')
        );
    }
        ...
        <div class="wrap about-wrap">
            ...
            <h2 id="woo-extra-cost-tab-wrapper" class="nav-tab-wrapper">
                ...
                foreach ($setting_tabs_wc as $name => $label)
                    echo '<a  href="' . home_url('wp-admin/index.php?page=woocommerce-social-share-discount-coupon&tab=' . $name) . '" class="nav-tab ' . ( $current_tab_wc == $name ? 'nav-tab-active' : '' ) . '">' . $label . '</a>';
                ?>
            </h2>
            ...
            <div class="return-to-dashboard">
                <a href="<?php echo home_url('/wp-admin/admin.php?page=wssdcpage'); ?>"><?php _e('Go to WooCommerce Social Share Discount Coupon Settings', 'woo-social-share-discount-coupon'); ?></a>
            </div>
        </div>
    ...
    public function woocommerce_social_share_discount_coupon_about() {
        ...
        <div class="changelog">
            ...
            <style type="text/css">
                p.woo_social_discount_coupon_overview {max-width: 100% !important;margin-left: auto;margin-right: auto;font-size: 15px;line-height: 1.5;}.woo_social_discount_coupon_content_ul ul li {margin-left: 3%;list-style: initial;line-height: 23px;}
            </style>  
        ...
        <?php
        $current_user = wp_get_current_user();
        if (!get_option('wssdc_plugin_notice_shown')) {
            ?>
            <div id="wssdc_dialog">
                ...
                <p><input type="text" id="txt_user_sub_wssdc" class="regular-text" name="txt_user_sub_wssdc" value="<?php echo $current_user->user_email; ?>"></p>
            </div>
            <?php
        }
    }

    public function woocommerce_social_share_discount_coupon_other_plugins() {
        global $wpdb;
