...
class Woo_Social_Share_Discount_Coupon_Public {
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script($this->plugin_name . 'one', plugin_dir_url(__FILE__) . 'js/woo-social-share-discount-coupon-public.js', array('jquery'), $this->version, false);
...
//                $output_fb = '<a id="fbShareButton" href="https://www.facebook.com/sharer/sharer.php?u=' . site_url() . '&title=' . $twit_desc . '"><img src="' . plugin_dir_url(__FILE__) . 'images/fb.png" alt=""/></a>';
        ...
        <script type="text/javascript">
            jQuery(document).on('click', '#fbShareButton', function($) {
        ...
        <?php $ajax_nonce = wp_create_nonce("WSSDC"); ?>
                var data = {
                    action: 'share_coupon_code',
                    value: 'yes',
                    security: '<?php echo $ajax_nonce; ?>',
                    orderData: '<?php echo $order; ?>',
                    userEmailAddress: '<?php echo isset($order->billing_email) ? $order->billing_email : ''; ?>'
                };
    ...
    public function share_coupon_code() {
        ...
        $digits = 6;
        $coupon_code = str_pad(rand(0, pow(10, $digits) - 1), $digits, '0', STR_PAD_LEFT);
        ...
        $is_sent = get_post_meta($_POST['orderData'], '_wsscd_sent', true);
